...
web.get("/admin", requireAdmin, async (req, res) => {
  const users = await listUsers();
  const platformGeminiKey = await getPlatformGeminiKey();
  const platformAiReady = Boolean(platformGeminiKey || process.env.GEMINI_API_KEY || process.env.ANTHROPIC_API_KEY);

  const rows = users
    .map((u) => {
      const ch = Boolean(u.meta?.igAccessToken || u.meta?.pageAccessToken || u.meta?.whatsappToken);
      const sub = statusInfo(u);
      return `<tr>
        <td><b>${esc(u.businessName || "-")}</b></td>
        <td style="color:var(--muted)">${esc(u.email)}</td>
        <td>${badge(Boolean(u.businessInfo))}</td>
        <td>${badge(ch)}</td>
        <td>${badge(ch)}</td>
        <td><span class="badge ${sub.active ? "on" : "off"}">${esc(sub.label)}</span></td>
        <td style="text-align:right"><a href="/admin/user/${u.id}" class="btn secondary" style="margin:0;padding:7px 14px;font-size:13px">Sozlash →</a></td>
      </tr>`;
    })
    .join("");

  const total = users.length;
  const connected = users.filter((u) => u.meta?.igAccessToken || u.meta?.pageAccessToken || u.meta?.whatsappToken).length;
  const activeSub = users.filter((u) => statusInfo(u).active).length;
  ...
  res.send(
    page(
      ...
      <div class="welcome">
        <h1>⚙️ Admin panel</h1>
        <p>Barcha bizneslar, tokenlar, narxlar va obunalarni shu yerdan boshqarasiz.</p>
        <div class="chips">
          <span class="chip">${platformAiReady ? "✓" : "•"} Platforma AI kaliti ${platformAiReady ? "sozlangan" : "yo'q"}</span>
          <span class="chip ${paymeReady ? "" : "warn"}">${paymeReady ? "✓" : "•"} Payme ${paymeReady ? "ulangan" : "ulanmagan"}</span>
        </div>
      </div>

      <div class="grid cols-3" style="margin-bottom:20px">
        <div class="card" style="margin:0"><div class="kpi"><div class="ic">🏢</div><div><div class="num">${total}</div><div class="lbl">Bizneslar</div></div></div></div>
        <div class="card" style="margin:0"><div class="kpi"><div class="ic">🔗</div><div><div class="num">${connected}</div><div class="lbl">Tarmoq ulangan</div></div></div></div>
        <div class="card" style="margin:0"><div class="kpi"><div class="ic">💳</div><div><div class="num">${activeSub}</div><div class="lbl">Faol obuna</div></div></div></div>
      </div>

      <div class="card">
        <div class="sec-title"><h2>🔑 Platforma Asosiy AI Kaliti (Google Gemini)</h2></div>
        <p class="hint">Bu kalit barcha bizneslar va foydalanuvchilar uchun yagona va asosiy Sun'iy Intellekt kaliti bo'lib xizmat qiladi. Foydalanuvchilar o'zi alohida kalit kiritishi shart emas.</p>
        <form method="post" action="/admin/gemini-key" style="display:flex; gap:10px; flex-wrap:wrap; align-items:flex-end">
          <div style="flex:1; min-width:300px">
            <label>Google Gemini API Key (AIzaSy...)</label>
            <input name="geminiApiKey" value="${esc(platformGeminiKey)}" placeholder="AIzaSy..." style="font-family:monospace" required>
          </div>
          <button style="margin:0">💾 Saqlash</button>
        </form>
      </div>
      ...
      <div class="card">
        ...
        <form method="post" action="/admin/plans">
          <div class="grid cols-3">
            ${Object.values(await getPlans())
              .map(
                (p) => `<div>
                <label>${esc(p.name)}</label>
                <input name="price_${esc(p.id)}" type="number" min="0" step="1000" value="${p.price}">
              </div>`
              )
              .join("")}
          </div>
          <button>💾 Narxlarni saqlash</button>
        </form>
      </div>

      <div class="card">
        <div class="sec-title"><h2>Bizneslar ro'yxati</h2><span class="tag">${total} ta</span></div>
        <div class="table-wrap">
          <table class="tbl">
            <thead><tr>
              <th>Biznes</th><th>Email</th><th>AI o'qitilgan</th><th>Tarmoqlar</th><th>Obuna</th><th></th>
            </tr></thead>
            <tbody>${rows || `<tr><td colspan="6" style="padding:16px;color:var(--muted)">Hali biznes yo'q</td></tr>`}</tbody>
          </table>
        </div>
      </div>`,
      { user: req.user, active: "admin" }
    )
  );
});

// Tarif narxlari o'zgartirish (admin)
web.post("/admin/plans", requireAdmin, async (req, res) => {
  const prices = {};
...
web.post("/admin/gemini-key", requireAdmin, async (req, res) => {
...
web.get("/admin/user/:id", requireAdmin, async (req, res) => {
  ...
  res.send(
    page(
      "Biznesni sozlash",
      `${saved ? `<div class="ok">O'zgarishlar saqlandi.</div>` : ""}
      <p style="margin-bottom:14px"><a href="/admin">← Barcha bizneslar</a></p>
      <div class="welcome">
        <h1>${esc(u.businessName || u.email)}</h1>
        <p>${esc(u.email)}</p>
        <div class="chips">
          <span class="chip ${u.businessInfo ? "" : "warn"}">${u.businessInfo ? "✓" : "•"} AI o'qitilgan</span>
          <span class="chip ${u.meta.igAccessToken || u.meta.pageAccessToken || u.meta.whatsappToken ? "" : "warn"}">${u.meta.igAccessToken || u.meta.pageAccessToken || u.meta.whatsappToken ? "✓" : "•"} Tarmoqlar</span>
          <span class="chip ${statusInfo(u).active ? "" : "warn"}">${esc(statusInfo(u).label)}</span>
        </div>
      </div>

      <div class="card">
        <h2 style="display:inline-flex; align-items:center; gap:10px">${brandIcon("instagram", { size: 26 })} Instagram (Instagram Login)</h2>
        <p class="hint">Instagram API with Instagram Login — Facebook sahifasi shart emas.
        Tokenni Meta App → Instagram → <b>API setup with Instagram login</b> → <b>Generate token</b> dan olasiz.</p>
        <form method="post" action="/admin/user/${u.id}/meta">
          <label>Instagram Access Token <span class="hint" style="font-weight:400">(IGAA… bilan boshlanadi)</span></label>
          <input name="igAccessToken" value="${esc(u.meta.igAccessToken)}" placeholder="IGAA...">
          <label>Instagram Business / User ID</label>
          <input name="igUserId" value="${esc(u.meta.igUserId)}" placeholder="17841400000000000">

          <h2 style="margin-top:22px; display:inline-flex; align-items:center; gap:10px">${brandIcon("facebook", { size: 26 })} Facebook Page (ixtiyoriy — eski usul)</h2>
          <p class="hint">Faqat Facebook Page orqali ulash kerak bo'lsa to'ldiring.</p>
          ...
          <h2 style="margin-top:22px; display:inline-flex; align-items:center; gap:10px">${brandIcon("whatsapp", { size: 26 })} WhatsApp</h2>
      ...
      <div class="card">
        ...
        <form method="post" action="/admin/user/${u.id}/subscription" style="display:flex;gap:10px;flex-wrap:wrap;align-items:end">
          <div style="min-width:160px"><label>Tarif</label>
            <select name="plan">
              ${Object.values(plans).map((p) => `<option value="${p.id}" ${u.subscription.plan === p.id ? "selected" : ""}>${esc(p.name)} — ${p.price.toLocaleString("uz")} so'm</option>`).join("")}
            </select></div>
          <button name="action" value="30" style="margin:0">+30 kun</button>
          <button name="action" value="365" class="secondary" style="margin:0">+365 kun</button>
          <button name="action" value="off" class="danger" style="margin:0">Bekor qilish</button>
        </form>
      </div>
...
web.post("/admin/user/:id/subscription", requireAdmin, async (req, res) => {
  const u = await findUserById(req.params.id);
  if (!u) return res.status(404).send("Biznes topilmadi");
  ...
  if (action === "off") {
    ...
    if (days > 0) await activate(u, days, plan);
  }
  res.redirect(`/admin/user/${u.id}?saved=1`);
});

web.post("/admin/user/:id/meta", requireAdmin, async (req, res) => {
  const u = await findUserById(req.params.id);
  if (!u) return res.status(404).send("Biznes topilmadi");
  await updateUser(u.id, {
    geminiApiKey: String(req.body.geminiApiKey || "").trim(),
    meta: {
      igAccessToken: String(req.body.igAccessToken || "").trim(),
